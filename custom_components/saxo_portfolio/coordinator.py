...
class SaxoCoordinator(DataUpdateCoordinator[Dict[str, Any]]):
    ...
    def __init__(self, hass: HomeAssistant, config_entry: ConfigEntry) -> None:
        ...
        self._api_client: Optional[SaxoApiClient] = None
    ...
    def _is_market_hours(self) -> bool:
        ...
        try:
            ...
            _LOGGER.error("Error checking market hours: %s", str(e))
        ...
        try:
            ...
            async with session.post(token_url, data=refresh_data, auth=auth) as response:
                if response.status == 200:
                    ...
                    _LOGGER.error("Token refresh failed: %s", error_data)
                    raise ConfigEntryAuthFailed("Failed to refresh access token")
        
        except Exception as e:
            _LOGGER.error("Error refreshing token: %s", str(e))
            raise ConfigEntryAuthFailed(f"Token refresh error: {str(e)}")
    ...
    async def _fetch_portfolio_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with async_timeout.timeout(COORDINATOR_UPDATE_TIMEOUT):
                ...
                # For now, we'll get this from the first account in the positions
                # In a real implementation, this would come from initial setup
                ...
                if positions_data.get("Data"):
                    ...
                    try:
                        ...
                        _LOGGER.warning("Could not fetch accounts data: %s", str(e))
                        # Create minimal account data from positions
                        accounts_data = {
                            "__count": 1,
                            "Data": [{
            ...
            _LOGGER.error("API error fetching portfolio data: %s", str(e))
    ...
    def get_account_sensor_data(self, account_id: str) -> Optional[Dict[str, Any]]:
    ...
    def get_position_sensor_data(self, position_id: str) -> Optional[Dict[str, Any]]:
