...
class SaxoApiClient:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    _LOGGER.warning(
                        "Network error %s: %s (attempt %d/%d), retrying in %d seconds", 
                        error_type, str(e), attempt + 1, MAX_RETRIES, backoff_time
                    )
                    await asyncio.sleep(backoff_time)
                    continue
                else:
                    raise APIError(f"{ERROR_NETWORK_ERROR} ({error_type}): {str(e)}")
    ...
    async def get_account_balance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.error("Error fetching account balance: %s", str(e))
            raise APIError(f"Failed to fetch account balance: {str(e)}")
    
    async def get_positions(self, client_key: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.error("Error fetching positions: %s", str(e))
    ...
    async def get_accounts(self, client_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.error("Error fetching accounts: %s", str(e))
