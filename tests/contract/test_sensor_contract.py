...
class TestSaxoSensorContract:
    ...
    def test_portfolio_sensor_state_schema(self, portfolio_sensor):
        ...
        assert hasattr(portfolio_sensor, "unique_id")
        assert hasattr(portfolio_sensor, "native_value")
        assert hasattr(portfolio_sensor, "extra_state_attributes")

        # Validate unique_id format (entity_id is generated by HA at runtime with has_entity_name)
        unique_id = portfolio_sensor.unique_id
        assert unique_id.startswith("saxo_123456_")
        assert unique_id == "saxo_123456_total_value"

        # Validate has_entity_name is set
        assert portfolio_sensor._attr_has_entity_name is True
        assert portfolio_sensor._attr_translation_key == "total_value"
    ...
    def test_multiple_sensor_types(self, mock_coordinator):
        ...
        # Should have different unique IDs (entity_id is generated by HA at runtime)
        unique_ids = {
            total_value_sensor.unique_id,
            cash_balance_sensor.unique_id,
            client_id_sensor.unique_id,
        }
        assert len(unique_ids) == 3  # All unique
    ...
    def test_sensor_data_type_consistency(self, portfolio_sensor):
        ...
        # This test MUST FAIL initially - no implementation exists
        # Get native_value multiple times (state requires HA entity platform registration)
        state1 = portfolio_sensor.native_value
        state2 = portfolio_sensor.native_value
